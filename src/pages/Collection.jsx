...
import bannerImage from "@/assets/banner.webp"; // Generic banner image
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
export default function Collection() {
  ...
  const uniqueSizes = [
    ...new Set(
      products.map((p) => p.size?.trim().toLowerCase()).filter(Boolean),
    ),
  ];
  const uniqueColors = [
    ...new Set(
      products.map((p) => p.color?.trim().toLowerCase()).filter(Boolean),
    ),
  ];
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center text-center p-4">
          <div className="animate-in slide-in-from-bottom-4 duration-1000 delay-100">
            ...
            <p className="text-base sm:text-lg text-white/90 max-w-2xl mx-auto drop-shadow-md">
              Explore our full range of premium travel gear designed for your
              every journey.
            </p>
          </div>
        </div>
                  ...
                  <DropdownMenu>
                    ...
                    >
                      <DropdownMenuItem
                        ...
                        className={`cursor-pointer ${sortBy === "default" ? "bg-primary/10 text-primary font-bold" : "font-medium"}`}
                      >
                        Featured
                      </DropdownMenuItem>
                      <DropdownMenuItem
