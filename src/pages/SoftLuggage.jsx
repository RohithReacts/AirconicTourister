...
import bannerImage from "@/assets/luggagebanner.webp";
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
export default function SoftLuggage() {
  useDocumentTitle("Soft Luggage");
  const [products, setProducts] = useState([]);
  const [allFetchedProducts, setAllFetchedProducts] = useState([]);
  const [loading, setLoading] = useState(true);
  const [sortBy, setSortBy] = useState("default");
  const [viewMode, setViewMode] = useState("soft-luggage");
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        const slProducts = allProducts.filter(
          (p) => p.category?.toLowerCase() === "soft-luggage",
        );
        setProducts(slProducts);
      } catch (error) {
    ...
    <div className="animate-in fade-in duration-500 w-full mb-12">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center text-center p-4">
          <div className="animate-in slide-in-from-bottom-4 duration-1000 delay-100">
            ...
            <h1 className="text-4xl sm:text-5xl lg:text-6xl font-extrabold tracking-tight mb-4 text-white drop-shadow-xl">
              {viewMode === "all" ? "All Products" : "Soft Luggage"}
            </h1>
            <p className="text-base sm:text-lg text-white/90 max-w-2xl mx-auto drop-shadow-md">
              Explore our versatile collection of soft luggage ensuring
              convenience and style for every journey.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="container mx-auto px-4 max-w-7xl">
        ...
        <nav className="flex items-center gap-2 text-sm text-muted-foreground mb-8 overflow-x-auto whitespace-nowrap pb-2 mt-2">
          ...
          <ChevronRight className="w-4 h-4 text-muted-foreground/50" />
          <span className="text-foreground font-semibold">Soft Luggage</span>
        </nav>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <DropdownMenu>
                    ...
                    >
                      ...
                      <DropdownMenuItem
                        onClick={() => {
                          setViewMode("soft-luggage");
                          setSortBy("default");
                        }}
                        className={`cursor-pointer ${viewMode === "soft-luggage" && sortBy === "default" ? "bg-primary/10 text-primary font-bold" : "font-medium"}`}
                      >
                        Featured
                      </DropdownMenuItem>
                      <DropdownMenuItem
